...
#		__logr_SCRIPT_LOG="${HOME}/logs/${__logr_LOG_NAME}.log"
	...
	# warn and error go to /var/log/system.log as well as logfile
		...
		__logr_exec $level "${__logr_LOG_NAME}" "${*:2}"
	# if no type is given, assumes info level
	else
		__logr_exec info "${__logr_LOG_NAME}:" "${*:1}"
	fi
}
...
__logr_exec() {
	...
	if [[ $__logr_VERBOSE == true ]]; then
		logger --no-act -p ${__logr_FACILITY}.$1 -t $2 -s $3 2>&1 | tee -a ${__logr_SCRIPT_LOG} 1>&2
	else
		logger --no-act -p ${__logr_FACILITY}.$1 -t $2 -s $3 2>> ${__logr_SCRIPT_LOG}
	fi
}
